...
// The catalog-project editor: a challenge built by hand (the catalog is
// mostly seeded from `projects/`). Admins only — everyone else creates a
// personal project at /projects/new.
export const load: PageServerLoad = async ({ locals, fetch, parent }) => {
  if (!locals.isAuthenticated) {
    throw redirect(303, "/login?next=/projects/new/challenge");
  }
  const { isAdmin } = await parent();
  if (!isAdmin) {
    throw redirect(303, "/projects/new");
  }
